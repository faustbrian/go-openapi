...
func TestExpandRejectsInvalidServerAndVariableInputs(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "non-string URL", value: serverValue(t, `{"url":42}`), options: server.DefaultOptions(), want: server.ErrInvalidServer},
		...
		{name: "non-string default", value: serverValue(t, `{"url":"https://{region}","variables":{"region":{"default":42}}}`), options: server.DefaultOptions(), want: server.ErrInvalidServer},
